  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3001/listings/books');
        ...
        setListings(response.data.filter(listing => listing.category === 'Books'));
        setLoading(false);
      } catch (error) {
        console.error('Error fetching book listings:', error);
        setError(error.message || 'Failed to fetch book listings');
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="container mt-4 text-center">
        ...
        <p className="mt-2">Loading book listings...</p>
      </div>
    );
  }
