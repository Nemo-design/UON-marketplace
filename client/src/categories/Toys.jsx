...
const Toys = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        const response = await axios.get('http://localhost:3001/listings/toys');
        if (!response.data) {
        ...
        setError('Failed to fetch toy listings');
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="container mt-5 text-center">
        ...
        <p className="lead text-primary">Loading toy listings...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container py-4">
      <div className="mb-4 text-center">
        <h1 className="display-5 fw-bold">Toy Listings</h1>
        <p className="lead text-muted">Find great deals on toys</p>
      </div>

      {!listings || listings.length === 0 ? (
        <div className="alert alert-info shadow-sm text-center">
          No toy listings found
        </div>
      ) : (
